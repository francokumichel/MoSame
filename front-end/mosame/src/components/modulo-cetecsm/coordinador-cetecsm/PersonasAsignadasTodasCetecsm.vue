<template>
    <div class="table-responsive table-content border border-secondary-subtle shadow">
        <PersonasAsignadasTodasCetecsmList :personasCetecsm="personasCetecsm" />
...
import PersonasAsignadasTodasCetecsmList from "./PersonasAsignadasTodasCetecsmList.vue";
...
export default {
    ...
    async created() {
        this.cargarPersonasAsignadasCetecsm();
    },

    components: { PersonasAsignadasTodasCetecsmList },
    ...
    methods: {
        async cargarPersonasAsignadasCetecsm() {
            try {
                const response = await apiService.get(import.meta.env.VITE_API_URL + "cetecsm/asignadas_todas" , {
                    params: {
                        page: this.page,
